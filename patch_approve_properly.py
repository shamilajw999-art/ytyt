...
pattern = re.compile(r"    suspend fun approvePaymentOrder\(.*?\): Pair<Boolean, String> = withContext\(Dispatchers\.IO\) \{.*?\n        \}\n    \}", re.DOTALL)

new_code = """    suspend fun approvePaymentOrder(
...
pattern2 = re.compile(r"    suspend fun rejectPaymentOrder\(.*?\): Boolean = withContext\(Dispatchers\.IO\) \{.*?\n        \}\n    \}", re.DOTALL)

new_code2 = """    suspend fun rejectPaymentOrder(
        orderId: String,
        adminUserId: String,
        targetUserId: String,
        reason: String
